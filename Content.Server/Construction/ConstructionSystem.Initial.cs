...
/// <summary>
            ...
            ICommonSession? senderSession = null)
        {
            ...
            if (!_actionBlocker.CanInteract(user, null)
                || (senderSession != null && EntityManager.TryGetComponent(user, out hands) && _handsSystem.GetActiveItem((user, hands)) == null)) // Goobstation, // HardLight: Merged with upstream; hands.ActiveHandEntity<_handsSystem.GetActiveItem((user, hands))
            {
                Cleanup();
                return false;
            }
            ...
            if (senderSession != null) // Goobstation - don't check this for constructor machine
            {
                ...
                if (_handsSystem?.GetActiveItem((user, hands)) is not { Valid: true } holding) // Goobstation, // HardLight: Merged with upstream; hands?.ActiveHandEntity<_handsSystem?.GetActiveItem((user, hands))
                {
                    Cleanup();
                    return false;
                }
