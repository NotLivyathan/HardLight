...
public sealed partial class SleepingSystem : EntitySystem
{
    ...
    private void OnDamageChanged(Entity<SleepingComponent> ent, ref DamageChangedEvent args)
    {
        ...
        if (args.DamageDelta.GetTotal() >= ent.Comp.WakeThreshold
            && !_statusEffect.HasEffectComp<ForcedSleepingStatusEffectComponent>(ent)) // Shitmed, // HardLight: Merged with upstream; replaced && !HasComp<ForcedSleepingComponent>(ent)
    ...
    public bool TrySleeping(Entity<MobStateComponent?> ent)
    {
        ...
        if (TryComp<AutoWakeUpComponent>(ent, out var autoWakeUp)) // Frontier: Set auto-wakeup time
            autoWakeUp.NextWakeUp = _gameTiming.CurTime + autoWakeUp.Length; // Frontier: ^
        return true;
    }
