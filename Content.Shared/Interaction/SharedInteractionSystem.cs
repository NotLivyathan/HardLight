...
namespace Content.Shared.Interaction
{
    ...
    public abstract partial class SharedInteractionSystem : EntitySystem
    {
        ...
        public bool InteractHand(EntityUid user, EntityUid target) // Goobstation: Useful return value; void<bool
        {
            if (IsDeleted(user) || IsDeleted(target))
                return false; // Goobstation: Added false

            var complexInteractions = _actionBlockerSystem.CanComplexInteract(user);
            if (!complexInteractions)
            {
                return InteractionActivate(user, // Goobstation: Added return
                    target,
                    checkCanInteract: false,
                    checkUseDelay: true,
                    checkAccess: false,
            ...
            // Frontier start: Adds extra things to the log, // HardLight: How incredibly descriptive. And there was no end tag, so I just have to guess where it should go.
